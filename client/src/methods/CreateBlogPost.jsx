...
Editor.CreatePost = (formData, request, id) => {
	// console.log(body)
	const state = {
		data: {},
		loading: false
	...
	console.log(request)
	if (request === "post") FormCreate("posts", setData, setLoading, formData, 'POST');
	else FormCreate(`posts/${id}`, setData, setLoading, formData, 'PATCH');
}
...
Editor.ProcessBlocks = (request, id) => {
...
Editor.EditBlogPost = async function EditBlogPost(id, setData, setLoading) {
	...
	submitButton.onclick = () => { Editor.ProcessBlocks('patch', id) }
